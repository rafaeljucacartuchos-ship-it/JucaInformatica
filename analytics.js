(function(){
  ...
  document.addEventListener('click',e=>{
    const el=e.target.closest('a,button,[data-brand]');if(!el)return;
    const href=el.getAttribute('href')||'';
    if(href.includes('wa.me')||el.id==='whatsappChatOpen'||el.classList.contains('whatsapp-chat-btn'))send('whatsapp_click',{label:(el.innerText||'WhatsApp').trim().slice(0,100),href});
    else if(el.matches('[data-brand]'))send('information_opened',{brand:el.dataset.brand,label:(el.innerText||'').trim().slice(0,100)});
    else if(href.startsWith('#'))send('navigation',{target:href,label:(el.innerText||'').trim().slice(0,100)});
    else if(el.tagName==='BUTTON')send('interaction',{label:(el.innerText||'').trim().slice(0,100)});
  },true);
})();