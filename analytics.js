(function(){
  const start=()=>{
    const KEY='juca_analytics_session'; let sid=sessionStorage.getItem(KEY); if(!sid){sid=(crypto.randomUUID?crypto.randomUUID():Date.now()+'-'+Math.random());sessionStorage.setItem(KEY,sid)}
    ...
    const ua=navigator.userAgent; const device=/Mobi|Android|iPhone|iPad/i.test(ua)?'Celular/tablet':'Computador'; const source=document.referrer?new URL(document.referrer).hostname:'Acesso direto';
    const send=(type,details={},duration=null)=>client.from('site_events').insert({session_id:sid,event_type:type,page,duration_seconds:duration,details:{...details,device,source}}).then(()=>{}).catch(()=>{});
